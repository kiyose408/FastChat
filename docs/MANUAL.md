...
FastChat是一款基于Qt框架开发的即时通讯应用，支持用户注册登录、好友管理、实时消息收发、文件传输等功能。
...
|----------|----------|
| 用户认证 | 用户注册、登录、JWT认证 |
| 好友管理 | 搜索用户、添加好友、删除好友、修改备注 |
| 实时通讯 | WebSocket实时消息收发、好友请求通知 |
| 文件传输 | 图片发送、文件发送、在线预览 |
...
1. 解压 `FastChat-0.1.0-beta.zip` 到任意目录
...
```json
[
  { "friend_id": 2, "username": "userA", "email": "a@email.com" }
]
```
